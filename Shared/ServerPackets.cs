...
namespace ServerPackets
{
    ...
    public sealed class G_HezeRequest : Packet { public override short Index { get { return (short)ServerPacketIds.G_HezeRequest; } } public uint RequesterID; public string RequesterName=""; protected override void ReadPacket(BinaryReader r) { RequesterID=r.ReadUInt32(); RequesterName=r.ReadString(); } protected override void WritePacket(BinaryWriter w) { w.Write(RequesterID); w.Write(RequesterName); } }
    public sealed class G_HezeAccepted : Packet { public override short Index { get { return (short)ServerPacketIds.G_HezeAccepted; } } public string PartnerName="", SkillName=""; public byte Level; protected override void ReadPacket(BinaryReader r) { PartnerName=r.ReadString(); SkillName=r.ReadString(); Level=r.ReadByte(); } protected override void WritePacket(BinaryWriter w) { w.Write(PartnerName); w.Write(SkillName); w.Write(Level); } }
    public sealed class G_HezeDeclined : Packet { public override short Index { get { return (short)ServerPacketIds.G_HezeDeclined; } } public string Name=""; protected override void ReadPacket(BinaryReader r) { Name=r.ReadString(); } protected override void WritePacket(BinaryWriter w) { w.Write(Name); } }
    public sealed class G_HezeBroken : Packet { public override short Index { get { return (short)ServerPacketIds.G_HezeBroken; } } public string Reason=""; protected override void ReadPacket(BinaryReader r) { Reason=r.ReadString(); } protected override void WritePacket(BinaryWriter w) { w.Write(Reason); } }
    public sealed class G_HezeEnergy : Packet { public override short Index { get { return (short)ServerPacketIds.G_HezeEnergy; } } public int Energy, Maximum; public string PartnerName=""; public byte Level; protected override void ReadPacket(BinaryReader r) { Energy=r.ReadInt32(); Maximum=r.ReadInt32(); PartnerName=r.ReadString(); Level=r.ReadByte(); } protected override void WritePacket(BinaryWriter w) { w.Write(Energy); w.Write(Maximum); w.Write(PartnerName); w.Write(Level); } }
    public sealed class G_HezeComboActivated : Packet { public override short Index { get { return (short)ServerPacketIds.G_HezeComboActivated; } } public string SkillName=""; public Point Location; public int Damage, Animation; protected override void ReadPacket(BinaryReader r) { SkillName=r.ReadString(); Location=new Point(r.ReadInt32(),r.ReadInt32()); Damage=r.ReadInt32(); Animation=r.ReadInt32(); } protected override void WritePacket(BinaryWriter w) { w.Write(SkillName); w.Write(Location.X); w.Write(Location.Y); w.Write(Damage); w.Write(Animation); } }
